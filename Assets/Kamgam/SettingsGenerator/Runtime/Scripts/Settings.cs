...
namespace Kamgam.SettingsGenerator
{
    ...
    public partial class Settings : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        /// <summary>
        /// Will use removeUnknownSettingsAfterLoad = false and provider = default provider for IConnectionWithProviderAccess.
        /// Use the other override(s) if you want to specify it.
        /// </summary>
        /// <param name="key"></param>
        /// <param name="settingsSaver"></param>
        public void Load(string key, SettingsSaverBase settingsSaver)
        {
            var provider = SettingsGeneratorSettings.GetOrCreate().Provider;
            Load(key, settingsSaver, removeUnknownSettingsAfterLoad: false, provider);
        }
        ...
        /// <param name="provider">The provider these settings are part of.</param>
        ...
        protected void postLoad(SettingsProvider provider)
        {
            ...
            foreach (var setting in _settingsCache)
            {
                ...
                if (setting.HasConnection())
                {
                    ...
                    // settings reference if connection is an IConnectionWithSettingsAccess
                    ...
                    // settings reference if connection is an IConnectionWithSettingsAccess
                    var connectionWithProviderAccess = connection as IConnectionWithProviderAccess;
                    if (connectionWithProviderAccess != null)
                    {
                        connectionWithProviderAccess.SetProvider(provider);
                    }
                }
        ...
        protected List<ISetting> getSettingsOrderedByConnectionOrderASC(IEnumerable<ISetting> settings)
        {
            ...
            _tmpSettingsSortedByConnectionOrder.Sort(compareByConnectionOrder);
...
#region sort by name/id
        ...
        /// <param name="connection"></param>
        /// <param name="provider">Optional (if not specified the default provider will be assumed).</param>
        /// <returns></returns>
        public SettingBool GetOrCreateBool(string id, bool defaultValue = false, List<string> groups = null, IConnection<bool> connection = null, SettingsProvider provider = null)
        {
            ...
            initConnectionForSetting(setting, connection, provider);
        ...
        protected void initConnectionForSetting<T>(ISettingWithConnection<T> setting, IConnection<T> connection, SettingsProvider provider)
        {
            if (connection != null)
            {
                ...
                if (connectionWithProviderAccess != null)
                {
                    if (provider == null)
                    {
                        provider = SettingsGeneratorSettings.GetOrCreate().Provider;
                    }
                    connectionWithProviderAccess.SetProvider(provider);
                }
