...
namespace Kamgam.SettingsGenerator
{
...
#endif
    {
        ...
        public Settings Settings
        {
            get
            {
                ...
                if (_settings == null || !_initialLoadDone)
                {
                    ...
                    _settings.Load(playerPrefsKey, SettingsSaver, RemoveUnknownSettingsAfterLoad, this);
...
#if ENABLE_INPUT_SYSTEM
        [Tooltip("Add your input action asset here and click the button below to generate settings for your input bindings. " +
                 "If you update your bindings then you have to click the button again to also update the settings (connections).\n\n" +
        ...
        public bool IncludeUIBindingsInAutoCreation = false;
        
        [Tooltip("If disabled then a search for all instances of the 'Input Action Asset' from above is made and " +
                 "the binding overrides are applied to all of them." +
                 "This is necessary because Unity makes lots of hidden copies of the asset during runtime.\n\n" +
        ...
        public bool DontApplyBindingOverridesToAllInstances = false;
...
#pragma warning restore  CS0414
        ...
        /// <summary>
        /// Resets all settings that are of type KeyCombination (old input system) or have an InputBindingConnection (new input system).
        /// </summary>
        ...
        /// <summary>
        /// Can be called even before the settings are loaded.\n
        /// If it is then custom save/load methods
        /// are ignored as they are obviously not existing yet.
        /// </summary>
        public void Delete()
        {
            if (_settings != null)
            {
                // Default behaviour
                _settings.Delete(playerPrefsKey, SettingsSaver);
            }
            else
            {
#if UNITY_EDITOR
                ...
                {
                    // Delete if not playing in Editor
...
#else
                // Delete if settings not yet loaded.
                SettingsSaver.Delete(playerPrefsKey);
#endif
            }
        }
        ...
        public static SettingsProvider EditorCreateProviderBasedOnTemplate(string providerPath, string name = null, bool pingAfterCreation = false)
        {
            ...
            if (template != null)
            {
                ...
                // Persist assets (it is important to save the settings first as the provider references them
                // and there was a bug that this reference was not saved if the provider asset was created first).
        ...
        public static void EditorSetProviderInScene(SettingsProvider provider, GameObject container = null)
        {
            ...
            foreach (var resolver in resolvers)
            {
                ...
                var resolverImpl = resolver as SettingResolver;
                ...
                var resolverProvider = resolver.GetProvider();
                if (resolverImpl != null)
                    resolverProvider = resolverImpl.GetProviderAsset();
                
                if (resolverProvider != provider)
                {
                    Logger.LogMessage($"Changing provider from '{resolverProvider}' to '{provider}' on '{resolverComp.name}'.", resolverComp.gameObject);
                    resolver.SetProvider(provider);
                    if (resolverImpl != null)
                        resolverImpl.SetProviderAsset(provider);
            ...
            // Update SettingsCheckForUnapplied
            var checkersForUnapplied = CompatibilityUtils.FindObjectsOfType<SettingsCheckForUnapplied>(includeInactive: true);
            foreach (var comp in checkersForUnapplied)
            {
            ...
            // Update SettingsButtonActions
            var buttonActions = CompatibilityUtils.FindObjectsOfType<SettingsButtonActions>(includeInactive: true);
            foreach (var comp in genericReceivers)
            {
                // Skip empty ones because its optional for button actions.
                if (comp.SettingsProvider == null)
                    continue;
