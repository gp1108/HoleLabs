...
namespace Kamgam.SettingsGenerator
{
    public class GameObjectInspector
    {
        ...
        private List<string> GetMemberPaths(object obj, string path, bool includePropsAndFields, bool includeMethods, bool getOrSetMethods, List<Type> compatibleTypes, List<string> results = null)
        {
            ...
            var fields = componentType.GetFields(BindingFlags | BindingFlags.NonPublic);
            ...
            if (includeMethods)
            {
                ...
                foreach (var method in methods)
                {
                    ...
                    {
                        // GET methods only
                        if (method.GetParameters().Length == 0 && (compatibleTypes == null || compatibleTypes.Contains(method.ReturnType)))
                        {
        ...
        public object GetAndCacheObjectAtPath(string path)
        {
            ...
            for (int i = 1; i < segments.Length; i++)
            {
                ...
                else
                {
                    ...
                    {
                        // Try field
                        FieldInfo field = type.GetField(segment, BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic);
                        if (field != null)
                        {
                            if (!field.IsPrivate || Attribute.IsDefined(field, typeof(SerializeField)))
                            {
