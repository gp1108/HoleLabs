...
namespace Kamgam.SettingsGenerator
{
    ...
    {
        /// <summary>
        /// The intensity that is used to simulate turned off SSAO (only if UseActiveStateToDisable == false).
        /// </summary>
        ...
        public override bool Get()
        {
            ...
            else
            {
                ...
                return intensity > OFF_INTENSITY;
            }
        }
        ...
        public override void Set(bool enable)
        {
            ...
            if (feature != null)
            {
                ...
                {
                    updateLastKnownIntensity(rpAsset);
                    float intensity = enable ? _lastKnownIntensities[rpAsset] : OFF_INTENSITY;
        ...
        protected void updateLastKnownIntensity(UniversalRenderPipelineAsset rpAsset)
        {
            ...
            // If the current intensity value is equal to the OFF_INTENSITY then abort here.
            // We only update the remembered value if it is NOT the OFF value as then it was
            // probably changed by the user.
            if (Mathf.Approximately(intensity, OFF_INTENSITY))
            {
                _lastKnownIntensities.TryAdd(rpAsset, 3); // Default intensity if intensity started with disabled value (no value is known).
            }
            else
            {
                _lastKnownIntensities[rpAsset] = intensity;
            }

#if UNITY_EDITOR
            // In the editor update the value used for play mode exit reset.
            if (backupIntensityValues.ContainsKey(rpAsset))
                backupIntensityValues[rpAsset] = intensity;
#endif
        }
...
#region Editor RenderPipelineAsset Revert
        ...
        protected static void onPlayModeExit(PlayModeStateChange state)
        {
            ...
            RenderPipelineRestoreEditorUtils.OnPlayModeExit<UniversalRenderPipelineAsset, float>(
                ...
                (rpAsset, value) =>
                {
                    // Reset on play mode exit only if in disabled state. Otherwise don't change the value
                    // as the user may have changed it during runtime and default Unity behaviour is to retain
                    // those changes, so we do too.
                    var currentValue = getIntensity(rpAsset);
                    if (Mathf.Approximately(0.001f, currentValue))
                        setIntensity(rpAsset, value);
                });
        }
#endif
