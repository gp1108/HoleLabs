...
namespace Kamgam.SettingsGenerator
{
    public class InputBindingConnection : Connection<string>, IConnectionWithProviderAccess
    {
        ...
        private static List<InputActionAsset> _tmpAssets = new List<InputActionAsset>();

        public override void Set(string overridePath)
        {
            var provider = GetProvider();
            var baseAsset = provider.InputActionAsset != null ? provider.InputActionAsset : _inputActionAsset;
            ...
            if (!provider.DontApplyBindingOverridesToAllInstances && baseAsset != null)
            {
                var assets = InputActionAssetUtils.FindInstancesOf(baseAsset, _tmpAssets, 0.2f);
                foreach (var asset in assets)
                {
                    overridePath = applyOverridesToActionAsset(asset, overridePath);
                    Logger.LogInEditorOnly("Applied override '" + overridePath + "' to binding id '" + _bindingId + "' in asset instance: " + asset.name + " instance id: " + asset.GetInstanceID());
                }
            }
            else
            {
                overridePath = applyOverridesToActionAsset(_inputActionAsset, overridePath);    
            }
        ...
        {
            InputBinding binding;
            bool found = inputActionAsset.FindBinding(_bindingId, out binding);
            if (found)
            {
                if (binding.isComposite)
                {
                ...
                else
                {
                    ...
                    var result = inputActionAsset.ApplyBindingOverrideWithResult(_bindingId, overridePath);
