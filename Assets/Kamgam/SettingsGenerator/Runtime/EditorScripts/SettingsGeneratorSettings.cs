...
namespace Kamgam.SettingsGenerator
{
    ...
    public class SettingsGeneratorSettings : ScriptableObject
    {
        ...
        static void onAfterSceneLoadAtRuntime()
        {
            ...
            if (config.Provider != null && !config.Provider.DisableAutoInitialization)
                config.InitializeAtRuntime();

            SceneManager.sceneLoaded += config.onSceneLoaded;
        }
        ...
        private void onSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            if (Provider.DisableAutoInitialization && !Provider.HasSettings())
                return;
        ...
        public static T GetOrCreateSetting<T>(string id, SettingData.DataType dataType) where T : class, ISetting
        {
            if (GetOrCreate().Provider == null)
                return null;

            return GetOrCreate().Provider.Settings.GetOrCreate(id, dataType) as T;
        }

        public static T GetSetting<T>(string id) where T : class, ISetting
        {
            return GetSetting(id) as T;
        }
        ...
        public static ISetting GetSetting(string id)
        {
            ...
            return GetOrCreate().Provider.Settings.GetSetting(id);
        }
        
        public static Settings GetSettings() => GetOrCreate().Provider != null ? GetOrCreate().Provider.Settings : null;
        public static SettingsProvider GetProvider() => GetOrCreate().Provider;
