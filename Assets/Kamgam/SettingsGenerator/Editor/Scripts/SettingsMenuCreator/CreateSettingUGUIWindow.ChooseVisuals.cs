...
namespace Kamgam.SettingsGenerator
{
    ...
    public partial class CreateSettingUGUIWindow : EditorWindow
    {
        ...
        protected VisualElement _createUIFromTemplateCtn;
        ...
        private VisualElement createChooseVisualGUI(VisualElement root)
        {
            ...
            _createUIFromTemplateCtn = ctn.AddContainer("UIFromTemplateCtn", "m-10", "bg-dark-grey", "mb-20", "dont-shrink");
            _createUIFromTemplateCtn.AddLabel("It seems there is no settings window yet in this scene.", "h2");
            _createUIFromTemplateCtn.AddLabel("Would you like to create a new settings window (choose below)?");
            var createWindowOptionsCtn = _createUIFromTemplateCtn.AddContainer("CreateWindowOptionsCtn", "horizontal", "dont-shrink");
            createWindowOptionsCtn.AddButton("Create PC Style Window", onCreatePcStyleUI, "ui-entry-create", "pt-10", "pb-10", "grow");
            createWindowOptionsCtn.AddButton("Create CONSOLE Style Window", onCreateConsoleStyleUI, "ui-entry-create", "pt-10", "pb-10", "grow");
        ...
        private void onResolversKeyUp(KeyUpEvent evt)
        {
            if (evt.keyCode == KeyCode.Delete)
            {
                ...
                foreach (var index in resolverIds)
                {
                    ...
                    Undo.RecordObject(resolver.SettingsProvider.SettingsAsset, "deactivated setting");
                    resolver.SettingsProvider.SettingsAsset.SetActive(resolver.GetID(), false);
                }
        ...
        // Activates all settings in the scene. Useful to ensure a setting that was deactivated before is now re-activated.
        ...
        private void updateCreateUIFromTemplateCtn()
        {
            bool settingsExist = openedScenesHaveSettings();
            _createUIFromTemplateCtn.SetDisplay(!settingsExist);
        }

        private bool openedScenesHaveSettings()
        {
            int sceneCount = EditorSceneManager.sceneCount;
            for (int i = 0; i < sceneCount; i++)
            {
        ...
        void onCreatePcStyleUI(ClickEvent evt)
        {
            onCreateUIFromTemplate("t:Prefab Settings_PC (Template)");
            _createUIFromTemplateCtn.SetDisplay(false);
        }
        
        void onCreateConsoleStyleUI(ClickEvent evt)
        {
            onCreateUIFromTemplate("t:Prefab Settings_CONSOLE (Template)");
            _createUIFromTemplateCtn.SetDisplay(false);
        }
        
        void onCreateUIFromTemplate(string templateSearchPath)
        {
            var guids = AssetDatabase.FindAssets(templateSearchPath);
            if (guids.Length == 0)
                return;
            var path = AssetDatabase.GUIDToAssetPath(guids[0]);
            var template = AssetDatabase.LoadAssetAtPath<GameObject>(path);

            var settings = PrefabUtility.InstantiatePrefab(template) as GameObject;
            if (settings == null)
                return;
            
            // Instantiate Prefab
            Undo.RegisterCreatedObjectUndo(settings, "Create Settings Window");
            PrefabUtility.UnpackPrefabInstance(settings, PrefabUnpackMode.OutermostRoot, InteractionMode.AutomatedAction);
            ...
            EditorSceneManager.SaveModifiedScenesIfUserWantsTo( new []{ settings.scene });
            
            // Select settings
            Selection.objects = new[] { settings };
        }
        
        static void addEventSystemIfNeeded()
        {
            ...
            bool addEventSystem = EditorUtility.DisplayDialog(
                "No event system found!",
                "There is no event system in the scene. Without an event system the UI will not react to any input.\n\n" +
                "Would you like to add an event system now?",
